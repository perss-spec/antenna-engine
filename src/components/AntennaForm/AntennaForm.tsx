  ...
  return (
    <div className="border-t border-border/30">
      <button
        ...
        className="w-full flex items-center justify-between px-6 py-4 text-[11px] font-semibold uppercase tracking-widest text-text-dim/70 hover:text-text-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
      >
        {title}
        <ChevronDown className={cn('w-4 h-4 transition-transform duration-200', open && 'rotate-180')} />
      </button>
      {open && (
        <div id={contentId} className="px-6 pb-5" style={{ animation: 'fadeIn 0.15s ease-out' }}>
          {children}
        </div>
      )}
    </div>
...
function UnitBadge({ unit }: { unit: string }) {
  return (
    <span className="shrink-0 inline-flex items-center px-2 h-9 text-xs text-text-dim bg-surface-hover border border-border/40 rounded-r border-l-0">
      {unit}
    </span>
  );
}
...
function FieldRow({ label, htmlFor, unit, children }: { label: string; htmlFor: string; unit?: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-2">
      <Label htmlFor={htmlFor} className="text-sm font-medium text-text-muted">{label}</Label>
    ...
    <div className={cn('flex flex-col', className)}>
      <form onSubmit={handleSubmit} className="flex flex-col">
        ...
        <div className="px-6 py-6 flex flex-col gap-8">
          <div className="flex flex-col gap-2">
            <Label htmlFor="antennaType" className="text-sm font-medium text-text-muted">Antenna Type</Label>
            <Select
              id="antennaType"
            ...
            {preset && (
              <p className="text-xs leading-relaxed text-text-dim/70 line-clamp-2 mt-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              {extraKBParams.slice(0, 6).map(([key, param]) => (
                <div key={key} className="flex flex-col gap-2">
                  <Label
                    htmlFor={`extra-${key}`}
                    className="text-sm font-medium text-text-muted truncate"
        ...
        <div className="px-6 py-6">
          <div className="relative">
            <Button
              type="submit"
              disabled={isSimulating}
              className="w-full h-10 bg-gradient-to-r from-accent via-accent to-purple-500 hover:opacity-90 text-white text-sm font-semibold shadow-lg shadow-accent/25 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60"
            >
              {isSimulating ? (
