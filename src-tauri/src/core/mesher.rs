...
use super::element::{AntennaElement, DipoleParams, PatchParams, QfhParams, MonopoleParams};
...
#[derive(Debug, Clone)]
pub struct MesherMesh {
    pub vertices: Vec<Point3D>,
    pub triangles: Vec<MesherTriangle>,
...
pub fn mesh_antenna(element: &AntennaElement, config: &MeshConfig) -> Result<MesherMesh, String> {
    match element {
        AntennaElement::Dipole(params) => mesh_dipole(params, config),
        AntennaElement::Patch(params) => mesh_patch(params, config),
        AntennaElement::Qfh(params) => mesh_qfh(params, config),
        AntennaElement::Monopole(params) => mesh_monopole(params, config),
    }
}

fn mesh_dipole(params: &DipoleParams, config: &MeshConfig) -> Result<MesherMesh, String> {
    let mut mesh = MesherMesh::new();

    // Dipole is a wire from center-length/2 to center+length/2 along orientation
    let half_len = params.length / 2.0;
    let orient = &params.orientation;
    let orient_norm = (orient.x * orient.x + orient.y * orient.y + orient.z * orient.z).sqrt();
    let dir = if orient_norm > 1e-15 {
        Point3D { x: orient.x / orient_norm, y: orient.y / orient_norm, z: orient.z / orient_norm }
    } else {
        Point3D { x: 0.0, y: 0.0, z: 1.0 }
    };
    let start = Point3D {
        x: params.center.x - half_len * dir.x,
        y: params.center.y - half_len * dir.y,
        z: params.center.z - half_len * dir.z,
    };
    let end = Point3D {
        x: params.center.x + half_len * dir.x,
        y: params.center.y + half_len * dir.y,
...
fn mesh_patch(params: &PatchParams, config: &MeshConfig) -> Result<MesherMesh, String> {
    let mut mesh = MesherMesh::new();

    let target_length = config.wavelength / config.elements_per_wavelength as f64;

    let nx = ((params.width / target_length).ceil() as usize).max(1);
    let ny = ((params.length / target_length).ceil() as usize).max(1);

    for j in 0..=ny {
        for i in 0..=nx {
            let x = params.center.x - params.width / 2.0 + (i as f64 * params.width) / nx as f64;
            let y = params.center.y - params.length / 2.0 + (j as f64 * params.length) / ny as f64;
            let z = params.center.z;
...
fn mesh_qfh(params: &QfhParams, config: &MeshConfig) -> Result<MesherMesh, String> {
    let mut mesh = MesherMesh::new();

    let circumference = std::f64::consts::PI * params.diameter;
    let helix_length = (params.turns * circumference.powi(2) + params.height.powi(2)).sqrt();
    ...
    for i in 0..=segments {
        ...
        let angle = 2.0 * std::f64::consts::PI * params.turns * t;
        let z = params.height * t + params.center.z;

        let point = Point3D {
            x: params.center.x + (params.diameter / 2.0) * angle.cos(),
            y: params.center.y + (params.diameter / 2.0) * angle.sin(),
            z,
        };
...
fn mesh_monopole(params: &MonopoleParams, config: &MeshConfig) -> Result<MesherMesh, String> {
    let mut mesh = MesherMesh::new();

    let target_length = config.wavelength / config.elements_per_wavelength as f64;
    let mut segments = (params.height / target_length).ceil() as usize;
    segments = segments.max(1);
    ...
    for i in 0..=segments {
        ...
        let point = Point3D {
            x: params.center.x,
            y: params.center.y,
            z: params.center.z + t * params.height,
        };
...
pub fn compute_mesh_quality(mesh: &MesherMesh) -> MeshQuality {
    ...
    for triangle in &mesh.triangles {
        ...
        let a = point_distance(p1, p2);
        let b = point_distance(p0, p2);
        let c = point_distance(p0, p1);
...
mod tests {
    ...
    #[test]
    fn test_mesh_dipole() {
        let element = AntennaElement::Dipole(DipoleParams {
            length: 1.0,
            radius: 0.001,
            center: Point3D { x: 0.0, y: 0.0, z: 0.0 },
            orientation: Point3D { x: 0.0, y: 0.0, z: 1.0 },
        });
    ...
    #[test]
    fn test_mesh_patch() {
        let element = AntennaElement::Patch(PatchParams {
            width: 1.0,
            length: 0.8,
            substrate_height: 0.016,
            substrate_er: 4.4,
            center: Point3D { x: 0.0, y: 0.0, z: 0.0 },
        });
    ...
    fn test_adaptive_wire_meshing() {
        let element = AntennaElement::Dipole(DipoleParams {
            ...
            radius: 0.001,
            center: Point3D { x: 0.0, y: 0.0, z: 0.0 },
            orientation: Point3D { x: 1.0, y: 0.0, z: 0.0 },
        });
        ...
        for i in 1..mesh.vertices.len() {
            let spacing = point_distance(&mesh.vertices[i - 1], &mesh.vertices[i]);
